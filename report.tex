...
\chapter{Introduction}
  ...
  \section{Linear Codes and Distance-Regular Graphs}
    ...
    Note that the distance graph $\Gamma_0$ is always the diagonal relation on
    its vertex set.  Furthermore, if $d$ is the diameter, then $\Gamma_0,
    \Gamma_1, \ldots, \Gamma_d$ partition the vertex set.

    Not only are the Hamming graphs distance-regular, but the graphs $H_i(d, q)$
    are the distance graphs of $H(d, q)$, and $d$ is the diameter.

    \mysep

    The easiest way to show that the Hamming graphs are distance-regular is to
    show that they satisfy a stronger property, called
    \textit{distance-transitivity}, which has to do with the symmetry of the
    Hamming graphs.

    \begin{defn}\label{defn:distance-transitive}
      An \textsc{automorphism} of a graph $\Gamma$ with vertex set $V$ is a
      bijection $\phi: V \to V$ such that for all vertices $u$ and $v$,
      $$
        u \sim v \iff \phi(u) \sim \phi(v) \ .
      $$

      A graph $\Gamma$ is \textsc{distance-transitive} if for all pairs of vertices $u,
      v$ and $u', v'$ such that $\dist(u, v) = \dist(u', v')$, there is an
      automorphism of $\Gamma$ mapping $u$ to $u'$ and $v$ to $v'$.
    \end{defn}
    ...
    \begin{lem}\label{lem:hamming-distance-transitive}
      For any positive integers $d, q$, the Hamming graph $H(d, q)$ is
      distance-transitive.
    \end{lem}
...
\chapter{Group Theory}
  ...
  \section{Character Theory}
    ...
    \begin{proof}
      ...
      $$
      for $q_i \in \N_{>0}$.  Since all finite abelian groups are isomorphic to
      a group of this form, it suffices to prove that each group of this form is
      isomorphic to its group of characters.
