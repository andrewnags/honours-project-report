...
\chapter{Association Schemes}
  ...
  \section{Partitions and Translation Schemes}
    ...
    \begin{proof}
      ...
      Since each $\col F_j$ is spanned by a disjoint set of characters,
      these subspaces are orthogonal and the $F_j$ are linearly independent;
      since together the characters span $\R^n$ (where $n$ is the order of
      $G$), the (direct) sum of the subspaces is $\R^n$ as well.  Therefore,
      $$
        I = F_0 + F_1 + \cdots + F_e
        \ .
      $$
      ...
      $$
        ...
        \ .
      $$
      Note that this implies that $d = \dim\BMA \leq \dim\mathbb{F} = e$.

      TODO: maybe we can remove (\ref{larger-row-partition})?
