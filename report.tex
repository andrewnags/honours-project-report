...
\chapter{Association Schemes}
  ...
  \section{Automorphisms and Cayley Graphs}
    ...
    the automorphisms of graphs (or schemes) allows to compute their eigenvalues
    ...
    the group operation, since the vertex set is also equal to the group.
    ...
    Before beginning the proof, it will be worthwhile to examine the neighbours
    ...
    \begin{proof}
      ...
      unique $g_u \in G$ such that $vg_u = u$.
  ...
  \section{Translation Schemes}
    ...
    In a sense, this section generalizes the latter result of the previous
    section to the setting of association schemes, using the former result.
    ...
    partitions into inverse-closed subsets of an abelian group which generate a
    translation scheme from those which do not. \cite[Section~12.10]{godsil}
    ...
    partition matrix $S$ then the \textsc{induced row partition} $\sigma^*$
    is the partition of the rows of $H$ such that two rows are in the same part
    if and only if the corresponding rows in $HS$ are equal.  In other words, if
    $f$ is the function which maps each row index $i$ of $H$ to the row vector
    $(HS)_i$, then $\sigma^*$ is the partition given by the fibres of $f$.
    ...
    \begin{proof}
      ...
      However, $\sigma^*$ is defined so that whever two rows of $HS$ are equal,
  ...
  \section{The Eigenvalues of the Hamming Scheme}
    ...
    $$
      ...
      \sqcup \set{0} \times \Zqz \times \set{0}^{d - 2}
      \sqcup \cdots \sqcup
      \set{0}^{d - 1} \times \Zqz 
      \ .
    $$
    ...
    For $\psi \in (\Zqd)^*$ and $x = (x_1, \ldots, x_d) = \sum_{i=1}^d x_i e_i$
    ...
    will yield a distinct character, by assigning this tuple to $\psi$
    (using the same identificationg used in (\ref{character-vector})),
    ...
    so that characters $\psi$ will be identified with row vectors
    $$
      \psi \rightsquigarrow
      \begin{bmatrix}
